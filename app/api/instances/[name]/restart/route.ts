...
export async function POST(
    ...
    try {
        ...
        // 2. Call Evolution API to restart
        const evolutionResponse = await fetch(`${apiUrl}/instance/restart/${name}`, {
            method: "POST",
            headers: {
                "apikey": apiToken,
            },
        ...
        if (!evolutionResponse.ok) {
            return NextResponse.json({ error: evoData.message || "Failed to restart" }, { status: evolutionResponse.status });
        }
